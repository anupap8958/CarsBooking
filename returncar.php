...
?>
    ...
    <div class="col-md-7" style="float: none; margin: 0 auto;">
      <div class="form-area">
        ...
        <br style="clear: both">
          <h3 style="margin-bottom: 5px; text-align: center; font-size: 30px;">รายละเอียด </h3>

           <h5> รถ :&nbsp;  <?php echo($car_name);?></h5>

           <h5> หมายเลขทะเบียนรถ :&nbsp;  <?php echo($car_nameplate);?></h5>

           <h5> วันเริ่มต้นการเช่า :&nbsp;  <?php echo($rent_start_date);?></h5>

           <h5> วันสิ้นสุดการเช่า:&nbsp;  <?php echo($rent_end_date);?></h5>

           <h5> อัตราค่าบริการ :&nbsp;<?php 
            if($charge_type == "days"){
                    echo ($fare . "/day");
                } else {
                    echo ($fare . "/km");
           ...
           <h5> พนักงานขับรถ :&nbsp;  <?php echo($driver_name);?></h5>

           <h5> เบอร์ติดต่อพนักงานขับรถ :&nbsp;  <?php echo($driver_phone);?></h5>
          <?php if($charge_type == "km") { ?>
          <div class="form-group">
            <input type="text" class="form-control" id="distance_or_days" name="distance_or_days" placeholder="กรุณากรอกระยะทางที่ใช้ไปทั้งหมด" required="" autofocus>
          </div>
          <?php }  else { ?>
            <h5> จำนวนวัน :&nbsp;  <?php echo($no_of_days);?></h5>
            <input type="hidden" name="distance_or_days" value="<?php echo $no_of_days; ?>">
          <?php } ?>
          <input type="hidden" name="hid_fare" value="<?php echo $fare; ?>">

           <input type="submit" name="submit" value="คืนรถ" class="btn btn-success pull-right">    
        </form>
      </div>
    </div>
